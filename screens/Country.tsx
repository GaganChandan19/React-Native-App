...
const Country = ({ navigation, route }: any) => {
  ...
  const [{ capital, latlng, population, flags }, setdata] = useState<IData>({
    capital: "",
    latlng: [],
    population: 0,
    flags: {},
  ...
  useEffect(() => {
    axios.get(`https://restcountries.com/v2/name/${country}?fullText=true`)
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Button
            testID="capitalButton"
          title="Capital Weather"
          onPress={() => navigation.navigate("Weather", { capital: capital })}
        ></Button>
      </View>
    </View>
  );
